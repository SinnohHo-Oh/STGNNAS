...
class SimpleNASController(torch.nn.Module): 
    ...
    def __init__(self, args, search_space, action_list, controller_hid=100, cuda=True, softmax_temperature=5.0, tanh_c=2.5):
        if not self.check_action_list(action_list, search_space):
            raise RuntimeError("There are actions not contained in search_space")
        #执行了self.check_action_list(action_list, search_space)
        ...
        self.encoder = torch.nn.Embedding(num_total_tokens, controller_hid) #116 100，每个元转化为一个100维向量表示
        ...
        for key in self.search_space:
            ...
            decoder = torch.nn.Linear(controller_hid, size) #比如 Linear(in_features=100, out_features=8, bias=True)
    ...
    def forward(self, inputs, hidden, action_name, is_embed):
        ...
        hx, cx = self.lstm(embed, hidden) #x_t是embed
        logits = self._decoders[action_name](hx)

        logits /= self.softmax_temperature #除一个值是分布更平稳，概率接近
    ...
    def sample(self, batch_size=1, with_details=False):
        ...
        for block_idx, action_name in enumerate(self.action_list):
           ...
           #logits 即 x_i, hidden 即 h_{i-1} 
             
            probs = F.softmax(logits, dim=-1) #P_i <-- Softmax(h_{i−1})
            log_prob = F.log_softmax(logits, dim=-1)

            entropy = -(log_prob * probs).sum(1, keepdim=False)
            action = probs.multinomial(num_samples=1).data #按照上面得出的分布随机采样出的一个值（id）
            selected_log_prob = log_prob.gather(1, utils.get_variable(action, requires_grad=False)) #将action对应的log_prob值拿出来

            entropies.append(entropy)
            log_probs.append(selected_log_prob[:, 0])

            inputs = utils.get_variable(action[:, 0] + sum(self.num_tokens[:decoder_index]), self.is_cuda, requires_grad=False) #把action对应的值加上sum(self.num_tokens[:decoder_index])再转化为Tensor，小id+前面所有部分的id，一维

            inputs = self.encoder(inputs) #1维，转化为100位Tensor

            actions.append(action[:, 0])

        actions = torch.stack(actions).transpose(0, 1)
        dags = self._construct_action(actions) #有向无环图
