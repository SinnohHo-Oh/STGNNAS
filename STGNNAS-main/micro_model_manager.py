...
from STGNN_models import STGNN_Conv
import torch
...
class MicroCitationManager(GeoCitationManager):
    ...
    def build_gnn(self, all_action): #√
        ...
        if (all_action['action'][1] == 'gcnconv'): #X
            self.Ks = 2 #不带matrix，在STGNN_Conv内由actions[1]来调整

        model = STGNN_Conv(self.args, all_action['action'], self.args.Kt, self.Ks, self.args.blocks, self.args.n_his, self.args.n_vertex).to(self.args.device) 
